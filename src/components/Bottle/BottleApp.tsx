...
export default function BottleApp() {
  ...
  const handleThrow = () => {
    if (!text.trim() && !imageUrl && !musicUrl) return;
    setIsThrowing(true);
    
    throwBottle(text, imageUrl, musicUrl);
    const currentText = text;
    addActivityLog({
      id: `${Date.now()}_bottle_throw`,
      title: '扔出漂流瓶',
      detail: (text || imageUrl || musicUrl || '发送了一个漂流瓶').slice(0, 60),
      timestamp: Date.now()
    });
    setText('');
    setImageUrl('');
    ...
    setTimeout(async () => {
      ...
      setTimeout(async () => {
        ...
        const prompt = `我扔了一个漂流瓶，内容是："${currentText}"。请你以DC角色${char.name}的身份，捡到这个漂流瓶并给出一个简短的回复。不要暴露你的真实身份，保持神秘感，但要符合你的性格。`;
        ...
        try {
          ...
          if (latestBottle) {
            ...
            // Add notification
            const { setNotification, setActiveApp } = useAppStore.getState();
            setNotification({
              id: Date.now(),
      ...
      <div className="flex-1 overflow-y-auto p-4 relative z-10 space-y-4">
        {bottles.map(bottle => (
          <div key={bottle.id} className="bg-white/10 backdrop-blur rounded-xl p-4 border border-white/20">
            ...
            {bottle.musicUrl && <div className="text-xs text-blue-300 bg-blue-900/30 p-2 rounded mb-3">🎵 {bottle.musicUrl}</div>}
            {bottle.reply ? (
        ...
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex flex-col justify-end">
          <div className="bg-white rounded-t-3xl p-6 text-gray-900 h-[70%] flex flex-col">
            <div className="flex justify-between items-center mb-4">
              ...
              <button 
                onClick={handleThrow} 
                disabled={(!text.trim() && !imageUrl && !musicUrl) || isThrowing}
            ...
            <input 
              ...
              className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm outline-none focus:border-blue-300 mb-4"
            ...
            <input 
              ...
              className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm outline-none focus:border-blue-300"
              placeholder="音乐 URL (可选)"
            />
          </div>
        </div>
      )}
