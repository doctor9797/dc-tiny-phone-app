...
function useRafLoop(cb: (now: number, dt: number) => void, active: boolean = true) {
...
function useAudioPlayer(tracks: Track[], callbacks?: Pick<MusicPlayerProps, 'autoPlay' | 'initialSeek' | 'onPlayStateChange' | 'onProgress' | 'onTrackChange'>) {
  ...
  const loadTrack = useCallback(
    (index: number, autoplay: boolean, direction: Direction) => {
      ...
      callbacks?.onTrackChange?.(tracks[index]?.id || '');
    },
    [tracks, playTransitionSound, getBandEnergy, callbacks]
  );
  ...
  useEffect(() => {
    ...
    const onTimeUpdate = () => {
      ...
      callbacks?.onProgress?.(audio.currentTime, audio.duration || 0);
    };
    const onEnded = () => {
  ...
  }, [state.loopMode, next, callbacks]);

  useEffect(() => {
    const audio = audioRef.current;
    if (!audio || !tracks.length) return;
    const newSrc = tracks[0].src;
    if (audio.src !== newSrc) {
      audio.src = newSrc;
      audio.load();
    }
  ...
  const handleAudioError = useCallback((e: React.SyntheticEvent<HTMLAudioElement>) => {
    console.warn('[widget audio error]', (e.currentTarget.src || '').slice(0, 80));
  }, []);
...
function Controls({
  ...
  return (
    <div className="controls" style={{ paddingBottom: 14 }}>
      <button
      ...
      >
        ...
        <span style={{
          ...
          fontSize: 7, color: shuffled ? '#fff' : 'transparent', transition: 'color 0.2s',
          whiteSpace: 'nowrap',
        }}>随机</span>
      </button>
      ...
      >
        ...
        <span style={{
          position: 'absolute', bottom: -10, left: '50%', transform: 'translateX(-50%)',
          fontSize: 7, color: loopMode !== 'off' ? '#fff' : 'transparent', transition: 'color 0.2s',
          whiteSpace: 'nowrap',
        }}>{loopMode === 'one' ? '单曲' : loopMode === 'all' ? '列表' : ''}</span>
      </button>
    </div>
  );
}
...
const MusicPlayer = React.memo(function MusicPlayer({
  ...
  const player = useAudioPlayer(tracks, { autoPlay, initialSeek, onPlayStateChange, onProgress, onTrackChange });
  const [isZoomed, setIsZoomed] = useState(false);
  const autoPlayHandled = useRef(false);

  // Autoplay on mount when transitioning from MusicApp to full player
  useEffect(() => {
    if (!autoPlay || autoPlayHandled.current) return;
    autoPlayHandled.current = true;
    const audio = player.audioRef.current;
    if (!audio) return;
    const tryPlay = () => {
      if (audio.paused && audio.src && audio.src !== location.href) {
        if (initialSeek && audio.duration && isFinite(audio.duration)) {
          audio.currentTime = Math.min(initialSeek, audio.duration);
        }
        audio.play().catch(() => {});
      }
    };
    setTimeout(tryPlay, 100);
  }, [autoPlay, initialSeek, player.audioRef, tracks]);
