// Cloudflare Worker — Netease Cloud Music weapi proxy
// Uses Web Crypto API (AES-CBC) + manual BigInt (raw RSA)

const IV = '0102030405060708';
const PRESET_KEY = '0CoJUm6Qyw8W8jud';
const BASE62 = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';

// RSA-1024 public key extracted from Binaryify/UnblockNeteaseMusic
const RSA_N = BigInt(
...
export default {
  async fetch(request) {
    ...
    if (request.method !== 'GET') {
      return cors(new Response(JSON.stringify({ code: -1, msg: 'Method not allowed' }), {
    ...
    try {
      ...
      if (!endpoint) {
        return cors(new Response(JSON.stringify({ code: -1, msg: 'Missing endpoint' }), {
      ...
      if (!config) {
        return cors(new Response(JSON.stringify({ code: -1, msg: 'Unknown endpoint: ' + endpoint }), {
          status: 400, headers: { 'Content-Type': 'application/json' },
        }));
      }
      ...
      return cors(new Response(JSON.stringify({ code: -1, msg: err.message }), {
